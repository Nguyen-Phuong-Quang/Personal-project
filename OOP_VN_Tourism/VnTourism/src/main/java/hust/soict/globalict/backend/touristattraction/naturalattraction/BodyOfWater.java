...
public class BodyOfWater extends NaturalAttraction {
	...
	public BodyOfWater() {
		super();
		// Each attribute will be initialized with the information we want to query,
		// will be used inside the sparql query.
	...
	public String createSparqlQuery() {
		...
		for (int i = 0; i < size; ++i) {
			if (i == 0)
				unionQuery = unionQuery + "{?place dbo:wikiPageWikiLink " + ObjectToCollect.OBJECT_BODY_OF_WATER[i]
						+ "}\r\n";
			else
